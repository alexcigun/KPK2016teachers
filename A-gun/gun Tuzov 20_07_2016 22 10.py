...
def check_for_hit():
    ...
    for ball in balls:
        ...
        for shell in shells_on_fly:
            ish=+1
            pass
            if abs(shell._x - ball._x) <= (shell._R + ball._R) and  \
               abs(shell._y - ball._y) <= (shell._R + ball._R):
               # print('Попали')
               shell_del.append(ish) # Запоминаем номера для удаления
               ball_del.append(ib)   # Запоминаем номера для удаления
               scores_value+=1
               scores_label.config(text='Ваши очёчки: '+str(scores_value))
               canvas.delete(ball._avatar)   # Удаляем аватары шаров
               canvas.delete(shell._avatar)  # Удаляем аватары снарядофф                            
               
    '''-------------------------------------------------------------------'''
    # Удаляем то, что запомнили
    # Удаляем шары
    for i in range(len(ball_del)):
        try:
            balls.pop(ball_del[i])
        except:
            pass
    # Удаляем снаряды
    for j in range(len(shell_del)):
        try:
            shells_on_fly.pop(shell_del[j])
        except:
            pass
