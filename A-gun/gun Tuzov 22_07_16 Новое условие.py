...
def check_for_hit():
    ...
    for i in range(len(shells_on_fly)):
        ...
        for j in range(len(balls)):
            if ((balls[j]._x+balls[j]._R)-(shells_on_fly[i]._x+shells_on_fly[i]._R))**2+((balls[j]._y+balls[j]._R)-(shells_on_fly[i]._y+shells_on_fly[i]._R))**2<(balls[j]._R+shells_on_fly[i]._R)**2:
