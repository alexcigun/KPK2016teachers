...
class Gun:
    ...
    def shoot(self):
        global shells_value,shells_value_label
        """
        ...
        shells_value+=1 # Увеличиваем количество выпущеных снарядов
        shells_value_label.config(text='Количество выстрелов: '+str(shells_value))
        print('Количество выстрелов: ',shells_value)
        return shell
    ...
    def change_tetta(self,event):
        ...
        if event.x!=0:
            ...
            self._lx =self._l* math.cos(tetta_r)
            self._ly =-self._l* math.sin(tetta_r)
            canvas.coords(self._avatar,self._x, self._y, self._x+self._lx,self._y+self._ly,)
...
def init_main_window():
    global root, canvas, scores_value,scores_label,  shells_value_label, shells_value # Количество выстрелов
    root = Tk()
    root.title("Пушка")
    scores_value=0
    shells_value=0
    ...
    shells_value_label=Label(root,text='Количество выстрелов: ')
    shells_value_label.grid(row=0, column=0)
...
def check_for_hit():
    ...
    for i in range(len(shells_on_fly)):
        ...
        for j in range(len(balls)):
            if ((balls[j]._x+balls[j]._R)-(shells_on_fly[i]._x+shells_on_fly[i]._R))**2+\
               ((balls[j]._y+balls[j]._R)-(shells_on_fly[i]._y+shells_on_fly[i]._R))**2<(balls[j]._R+shells_on_fly[i]._R)**2:
            # if (shells_on_fly[i]._x - balls[j]._x)**2+(shells_on_fly[i]._y - balls[j]._y)**2 <= (shells_on_fly[i]._R + balls[j]._R)**2:
                print('Попали')
                print('Сколько снарядов осталось: ',len(shells_on_fly),' Сколько мячей осталось: ',len(balls))
                # Добавляем количество очков в зависимости от радиуса
                if balls[j]._R<22:
                    scores_value+=3
                elif balls[j]._R<32:
                    scores_value+=2
...
def timer_event():
    ...
    for shell in shells_on_fly:
        ...
        # ------ Проверка на вылет за границу холста ------------
        if shell._x>screen_width+10 or shell._x<-10 or shell._y>screen_height+10 or shell._y<-10:
        ...
        # -------------------------------------------------------
    for j in range(len(balls_del)):
        canvas.delete(shells_on_fly[j]._avatar)
...
def click_event_handler(event):
    ...
    print('Сколько всего снарядов в воздухе',len(shells_on_fly))
    print('Сколько всего шаров в воздухе',len(balls))
